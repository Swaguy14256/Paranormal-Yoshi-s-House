...
UnpauseMusic:
	mov a, #$00
	mov !PauseMusic, a
	mov	a, !PlayingVoices		; Loads the currently playing channels.
	eor	a, $1d				; Removes the channels occupied by sound effects.
	call	KeyOnVoices			; Calls the key on voices routine.
.unsetMute:
	mov a, !SpeedUpBackUp	;\
...
L_099C:
	...
	mov	y, #$20		; and we ESPECIALLY don't want the echo buffer to overwrite anything.
	movw	$f2, ya
	mov	!NCKValue, y
	
;	mov	a, #$ff
	mov	a, $1d				; Loads the channels used by sound effects.
	eor	a, #$ff				; Removes the channels unoccupied by sound effects.
	call	KeyOffVoices
...
if !noSFX = !false
	...
	cmp	$06, #$08			; |
	beq	.KEEPECHOVOLUME			;/
	bra	.RESETMASTERVOLUME		; Branches to the reset master volume sublabel.
.KEEPECHOVOLUME
	mov	!MusicEchoChannels, #$f6	; Makes channels 1, 2, and 4-7 have echo.
.RESETMASTERVOLUME	
	push	a				; Preserves the A Register.
	mov	a, #$7f				; Loads the left master volume.
	mov	y, #$0c				; Loads the DSP Register to write to.
	call	DSPWrite			; Calls the DSP Register write routine.
	mov	a, #$7f				; Loads the right master volume.
	mov	y, #$1c				; Loads the DSP Register to write to.
	call	DSPWrite			; Calls the DSP Register write routine.
	pop	a				; Pops back the A Register.
	; MODIFIED CODE START
	mov	y, #$10
-	
