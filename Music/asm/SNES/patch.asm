...
org $806B
	JML HANDLESFX		; Jumps to the SFX handling routine.
org $8075
	JML MainLabel
...
HANDLESFX:
	LDA $10			;\
	BEQ SFXEND		;/ Branches if the game is not running.
	PHP			; Preserves the processor flags.
	PHB			;\
	PHK			; | Preserves and changes the Data Bank Register.
	PLB			;/
	LDA !SpecialMir
	STA !SpecialReg
	LDA !SFX1DF9Mir
...
if !PSwitchStarRestart == !true	
	...
	PLB			; Restores the old Data Bank Register.
	PLP			; Pulls back the processor flags.
	JML $80806F		; Jumps back to original code.
MainLabel:
	STZ $10
	PHP
	PHB
...
if !PSwitchStarRestart == !true
	...
	LDA $0DDB		;\
	BNE ChangeMusic		;/ Branches if the force music reload flag is set.
End:	
	LDA $0100		;\
	CMP #$12		; | Branches if a level is not being loaded.
	BNE NOMUTE		;/
	JSR MUTEMUSIC		; Jumps to the mute music routine.
NOMUTE:
	CLI
	PLB
	PLP
...
SFXEND:
	JML $80806B		; Return.  TODO: Detect if the ROM is using the Wait Replace patch.
...
+
	LDA $0100		;\
	CMP #$0B		; | Branches if there is a transition from the level to overworld.
	BEQ .STARTUPLOAD	;/
	CMP #$0F		;\
	BEQ .STARTUPLOAD	;/ Branches if there is a fade to level.
	CMP #$13		;\
	BEQ .STARTUPLOAD	;/ Branches if there is a level fade in.
	CMP #$14		;\
	BEQ .STARTUPLOAD	;/ Branches if Mario is in a level.
	CMP #$15		;\
	BEQ .STARTUPLOAD	;/ Branches if there is a transition from the level to the Game Over or Time Up screen.
...
;NoUpload:
	...
	STZ $10			; Resumes the game
	LDA #$81		;\
	TSB $4200		;/ Enables NMI and controller inputs.
;	STA $4200
	JMP End
...
MUTEMUSIC:
	...
	BNE MUTEEND		; Branches if Mario is not in level 31.
	LDA #$0A		;\
