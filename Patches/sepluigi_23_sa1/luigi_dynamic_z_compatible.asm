...
!Bank = #$1D		; The bank byte for Luigi's graphics.
!Address = #$8008	; The address for Luigi's graphics + 8.
incsrc header.asm
...
sa1rom
!addr = $6000
!base = $000000		; The base address in SA-1 ROMS.
else
lorom
!addr = $0000
!base = $800000		; The base address in LoROM ROMS.
endif
...
;	STA $4324
	LDA #$01			;\
	STA !marioNormalCustomGFXOn	;/ Enable custom Mario graphics.

	LDA !Bank			;\
	STA !marioNormalCustomGFXBnk	;/ Sets the bank to upload the custom graphics from.

	REP #$20			; Turns on 16-bit addressing for the Accumulator.
	LDA !Address			;\
	STA !marioNormalCustomGFXRec	;/ Sets the address to upload the custom graphics from.
