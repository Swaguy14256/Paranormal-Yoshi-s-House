...
.DontJump3
	LDA #$F0
	%speedup($FC)		;\
	%speedup($F8)		;/ Jumps to the slot checking macros for slots $03F9 and $03F5. Lakitu's Cloud is not used in this case anyway, and issues arise without these extra slots in Paranormal Yoshi's House.
	%speedup($F4)
	%speedup($F0)
