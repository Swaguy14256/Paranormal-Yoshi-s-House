...
CustExObj98:
	LDA $13BF		; Gets the current translevel number.
	TAX			; Transfers the Accumulator to the X Register.
	LDA $7FB4D9,x		;\
	CMP #$01		; | Branches if the midpoint number is greater than or equal to the one being set.
	BCS DONOTSETMIDWAY	;/
	LDA #$00		; Loads the index for the 2 tiles to load for a 2x1 object.
	JSR Objects2x1		; Jumps to the 2x1 object routine.
DONOTSETMIDWAY:
	RTS			; Ends the code
CustExObj99:
	LDA $13BF		; Gets the current translevel number.
	TAX			; Transfers the Accumulator to the X Register.
	LDA $7FB4D9,x		;\
	CMP #$02		; | Branches if the midpoint number is greater than or equal to the one being set.
	BCS DONOTSETMIDWAY	;/
	LDA #$01		; Loads the index for the 2 tiles to load for a 2x1 object.
	JSR Objects2x1		; Jumps to the 2x1 object routine.
	RTS			; Ends the code.
CustExObj9A:
	LDA $13BF		; Gets the current translevel number.
	TAX			; Transfers the Accumulator to the X Register.
	LDA $7FB4D9,x		;\
	CMP #$03		; | Branches if the midpoint number is greater than or equal to the one being set.
	BCS DONOTSETMIDWAY	;/
	LDA #$02		; Loads the index for the 2 tiles to load for a 2x1 object.
	JSR Objects2x1		; Jumps to the 2x1 object routine.
CustExObj9B:
CustExObj9C:
CustExObj9D:
...
; 00 - "cluster" object (i.e., always has the same arrangement of tiles but without a pattern)
CustObj00:
	LDA $59
	STA !ObjScratch
	LDA $58
	JMP ClusterExObjectsEntry1

; 01 - object with 4 distinct sides
CustObj01:
	LDA $58
	JMP SquareObjects

; 02 - object with 4 distinct sides; automatically changes when placed over non-blank tiles
CustObj02:
	LDA $58
	JMP SquareObjects2

; 03 - object that can only be stretched in one direction
CustObj03:
	LDA $58
	JMP UnidimensionalObjects

; 04 - object that can only be stretched in one direction; automatically changes when placed over non-blank tiles
CustObj04:
	LDA $58
	JMP UnidimensionalObjects2

; 05 - object that can only be stretched in one direction and is 2 tiles wide
CustObj05:
	LDA $58
	JMP UnidimensionalObjectsWide

; 06 - steep slope left
CustObj06:
; 07 - steep slope right
CustObj07:
; 08 - normal slope left
CustObj08:
; 09 - normal slope right
CustObj09:
; 0A - gradual slope left
CustObj0A:
; 0B - gradual slope right
CustObj0B:
; 0C - upside-down steep slope left
CustObj0C:
; 0D - upside-down steep slope right
CustObj0D:
; 0E - upside-down normal slope left
CustObj0E:
; 0F - upside-down normal slope right
CustObj0F:
	LDA $5A
	SEC
	SBC #$06
...
; 10 - ground
CustObj10:
; 11 - ground with left corner
CustObj11:
; 12 - ground with right corner
CustObj12:
; 13 - ground with left and right corners
CustObj13:
; 14 - ceiling
CustObj14:
; 15 - ceiling with left corner
CustObj15:
; 16 - ceiling with right corner
CustObj16:
; 17 - ceiling with left and right corners
CustObj17:
; 18 - left wall
CustObj18:
; 19 - right wall
...
CustObj1B:
; 1C - ground and floor with dirt in between
CustObj1C:
; 1D - left and right walls with dirt in between
CustObj1D:
; 1E - 4-sided block of ground
CustObj1E:
; 1F - block of dirt with inside corners
CustObj1F:
	LDA $5A
	SEC
	SBC #$10
...
Obj2x1Tiles:
	dw $0035,$031F,$0035,$0318,$0035,$0405
