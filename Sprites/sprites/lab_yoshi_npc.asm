...
!MessagePalette = $90		; Which palette to read from for messages. Palette E by default.
...
!2ndMessage = $2D		; The translevel number for the 2nd message.

!3rdMessage = $2E		; The translevel number for the 3rd message.

!MessageNumber1 = $02		; The 1st and 3rd message numbers for the hardcoded messages.

!MessageNumber2 = $01		; The 2nd message number for the hardcoded messages.

!1stMusicBlockMessage = $3D	; The translevel number for the 1st music block message.

!2ndMusicBlockMessage = $52	; The translevel number for the 2nd and 3rd music block messages.
...
!Message3Timer = $1540			; The RAM used for the 3rd message timer.
!Message4Timer = $154C			; The RAM used for the 4th message timer.
...
LDA $010B				;\
CMP #$004A				;/ Checks if Mario is in level 4A.
SEP #$20				; Turns on 8-bit addressing mode.
BNE REGULAR				; Branches if Mario is not in level 4A. This is hardcoded for Paranormal Yoshi's House.
LDA #$00				;\
STA !ExtraProperty1,x			;/ Makes the NPC stationary.
REGULAR:
...
AND #$02
BEQ NOReturnZ				; Branches if the sprite is set to display no message.
ReturnZZ:
JMP ReturnZ				; Jumps to the return address.
...
AND #$04				;/
BNE LEVELISINCOMPLETE			; Branches if the extra bit is set. The below routine is hardcoded.

LDA $1ECA				;\
AND #$80				; | Branches if level 104 is incomplete.
BEQ LEVELISINCOMPLETE			;/

LDA !Message2_Timer,x			;\ Handle second message if needed
BEQ NoMusicMessage1			;/
LDA $7C					;\
CMP #$3E				; | Branches if the level number does not match the one used by this sprite.
BNE RESUMEMOVEMENT2			;/
JMP MusicMessage1			; Jumps to a harcoded message sequence.

RESUMEMOVEMENT2:
JMP RESUMEMOVEMENT			; Jumps to the resume movement routine.

NoMusicMessage1:
LDA !Message3Timer,x			;\
BEQ NoMusicMessage2			;/ Handles the 3rd message if it is needed. It branches if that timer is 0.
JMP MusicMessage2			; Jumps to another harcoded message sequence.

NoMusicMessage2:
LDA !Message4Timer,x			;\
BEQ LEVELISINCOMPLETE			;/ Handles the 4th message if it is needed.  It branches if that timer is 0.
JMP MusicMessage3			; Jumps to yet another harcoded message sequence.
...
LDA !Message3Timer,x			;\
BEQ NoMessage3				;/ Handles the 3rd message if it is needed. It branches if that timer is 0.
JMP Message3				; Jumps to one more harcoded message sequence. This one messes with the music.
...
STA $7C					; Stores the level number to an overlapping level number address.
LDA $09
...
LDA #!2ndMessage			;\
STA $13BF|!Base2			;/ Sets the translevel number for the 2nd message.
	
LDA #!MessageNumber1			;\
STA $1426|!Base2			;/ Displays the 2nd message.
STZ !Message2_Timer,x			; Clears the message 2 timer.

LDA #$02				;\ Sets the message 3 timer.
STA !Message3Timer,x			;/
BRA ReturnZ				; Branches to the Return address.
...
JSR TALK				; Jumps to the facing routine.
LDA #$2F				;\
STA $1DFB				;/ Sets the music to play. Again, it is hardcoded.

LDA #!3rdMessage			;\
STA $13BF|!Base2			;/ Sets the translevel number for the 3rd message.

LDA #!MessageNumber2			;\
STA $1426|!Base2			;/ Displays the 3rd message.
STZ !Message3Timer,x			; Clears the message 3 timer.

PHX					; Preserves the X Register.
LDA $5B					;\
AND #$01				; | Makes the screen mode vertical with layers 1 and 2. 
ASL					;/
TAX					; Transfers the Accumulator to the X Register.
LDA $95,x				; Loads the high byte of the player X Position.
TAX					; Transfers the Accumulator to the X Register.
LDA $19B8+$03				;\
STA $19B8,x				; |
LDA $19D8+$03				; | Sets the screen exit.
STA $19D8,x				;/
...
STZ $7B					; Sets Mario's speed to 0.
JSR TALK				; Jumps to the facing routine.
LDA #!2ndMusicBlockMessage		;\
STA $13BF|!Base2			;/ Sets the translevel number for the 3rd message.

LDA #!MessageNumber2			;\
STA $1426|!Base2			;/ Displays the 3rd message.
STZ !Message3Timer,x			; Clears the message 3 timer.
LDA #$02				;\ Sets the message 4 timer.
STA !Message4Timer,x			;/
JMP ReturnZ				; Jumps to the Return address.
...
JSR TALK				; Jumps to the facing routine.
LDA #!2ndMusicBlockMessage		;\
STA $13BF|!Base2			;/ Sets the translevel number for the 4th message.

LDA #!MessageNumber1			;\
STA $1426|!Base2			;/ Displays the 4th message.
STZ !Message4Timer,x			; Clears the message 4 timer.
JMP RESUMEMOVEMENT			; Jumps to the resume movement routine.
...
FramesTotalTiles:
	db $09,$08,$0A,$08,$0A,$09,$09,$09,$09,$08,$0A,$08,$0A,$09,$09,$09
StartPositionFrames:
	dw $0009,$0012,$001D,$0026,$0031,$003B,$0045,$004F,$0059,$0062,$006D,$0076,$0081,$008B,$0095,$009F
EndPositionFrames:
	dw $0000,$000A,$0013,$001E,$0027,$0032,$003C,$0046,$0050,$005A,$0063,$006E,$0077,$0082,$008C,$0096

FramesXDisp:
Walk1XDisp:
	db $01,$F9,$01,$F9,$09,$09,$09,$05,$01,$01
Walk2XDisp:
	db $00,$F8,$00,$F8,$08,$09,$09,$01,$00
Walk3XDisp:
	db $FC,$00,$07,$07,$00,$F8,$00,$09,$F8,$01,$F9
Walk4XDisp:
	db $00,$F8,$00,$F8,$08,$09,$09,$01,$00
Walk5XDisp:
...
Idle3XDisp:
	db $01,$F9,$01,$F9,$09,$09,$09,$05,$01,$01
Walk1FlipXXDisp:
	db $FF,$07,$FF,$0F,$FF,$FF,$FF,$03,$07,$07
Walk2FlipXXDisp:
	db $00,$08,$00,$10,$00,$FF,$FF,$07,$08
Walk3FlipXXDisp:
	db $0C,$08,$01,$01,$00,$08,$00,$FF,$10,$07,$0F
Walk4FlipXXDisp:
	db $00,$08,$00,$10,$00,$FF,$FF,$07,$08
Walk5FlipXXDisp:
...
FramesYDisp:
Walk1YDisp:
	db $EA,$EA,$FA,$FA,$0A,$F2,$EE,$EC,$F2,$0A
Walk2YDisp:
	db $EB,$EB,$FB,$FB,$08,$F3,$EF,$F3,$08
Walk3YDisp:
	db $F5,$F4,$F3,$EF,$EC,$EC,$FC,$08,$00,$08,$08
Walk4YDisp:
	db $EB,$EB,$FB,$FB,$08,$F3,$EF,$F3,$08
Walk5YDisp:
...
Idle1YDisp:
	db $EA,$EA,$FA,$FA,$0A,$F2,$EE,$EC,$F2,$0A
Idle2YDisp:
	db $EB,$EB,$FB,$FB,$0A,$F3,$EF,$ED,$F3,$0A
Idle3YDisp:
	db $EB,$EB,$FB,$FB,$0A,$F3,$EF,$ED,$F3,$0A
Walk1FlipXYDisp:
	db $EA,$EA,$FA,$FA,$0A,$F2,$EE,$EC,$F2,$0A
...
FramesProperty:
Walk1Properties:
	db $46,$46,$46,$46,$42,$42,$42,$42,$42,$42
Walk2Properties:
	db $46,$46,$46,$46,$42,$42,$42,$42,$42
Walk3Properties:
	db $42,$42,$42,$42,$46,$46,$46,$42,$46,$42,$42
Walk4Properties:
	db $46,$46,$46,$46,$42,$42,$42,$42,$42
Walk5Properties:
	db $42,$42,$42,$46,$46,$46,$46,$46,$42,$42,$42
Idle1Properties:
	db $46,$46,$46,$46,$42,$42,$42,$42,$42,$42
Idle2Properties:
	db $46,$46,$46,$46,$42,$42,$42,$42,$42,$42
...
Walk1FlipXProperties:
	db $06,$06,$06,$06,$02,$02,$02,$02,$02,$02
Walk2FlipXProperties:
	db $06,$06,$06,$06,$02,$02,$02,$02,$02
Walk3FlipXProperties:
	db $02,$02,$02,$02,$06,$06,$06,$02,$06,$02,$02
Walk4FlipXProperties:
	db $06,$06,$06,$06,$02,$02,$02,$02,$02
Walk5FlipXProperties:
	db $02,$02,$02,$06,$06,$06,$06,$06,$02,$02,$02
...
FramesTile:
Walk1Tiles: 
	db $A6,$A7,$A9,$AB,$BB,$AC,$AC,$AC,$AD,$BC
Walk2Tiles: 
	db $A6,$A7,$A9,$AB,$BB,$AC,$AC,$AD,$BC
Walk3Tiles: 
	db $AB,$AB,$AB,$AB,$A6,$A7,$A9,$BB,$AD,$BC,$BD
Walk4Tiles: 
	db $A6,$A7,$A9,$AB,$BB,$AC,$AC,$AD,$BC
Walk5Tiles: 
...
Walk3Size:
	db $00,$00,$00,$00,$02,$02,$02,$00,$00,$00,$00
Walk4Size: 
	db $02,$02,$02,$00,$00,$00,$00,$00,$00
Walk5Size: 
...
resource:
incbin lab_yoshi_npc.bin ; Replace this for the name of your GFX.
